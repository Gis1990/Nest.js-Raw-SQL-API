...
export class UsersQueryRepository {
    ...
    async getAllUsers(dto: ModelForGettingAllUsers): Promise<UsersClassPaginationDto> {
        ...
        whereClause += ` ORDER BY "${sortBy}" COLLATE "C" ${sort} LIMIT $${paramsCounter} OFFSET $${paramsCounter + 1}`;
    ...
    async GetAllBannedUsersForBlog(
        ...
        let whereClauseForCount = "";
        if (searchLoginTerm) {
            whereClause += `AND login ILIKE $3 ORDER BY "${sortBy}" COLLATE "C" ${sort} LIMIT $4 OFFSET $5`;
